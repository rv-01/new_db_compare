...
class DatabaseComparator:
    ...
    def fetch_data_batchwise(self, db_config: DatabaseConfig, table_config: TableConfig, 
        ...
        """Fetch data from database in batches using ROWNUM"""
        ...
        # Use ROWNUM for pagination (Oracle-specific)
        sql = f"""
        SELECT {column_list}
        FROM (
            SELECT {column_list}, ROWNUM rn
    ...
    def hash_rows(self, df: pd.DataFrame, primary_keys: List[str]) -> Dict[str, str]:
        ...
        for _, row in df.iterrows():
            ...
            # Create hash of all column values (excluding rownum if present)
            row_data = row.drop('rn', errors='ignore')
    ...
    def generate_sql_statements(self, table_config: TableConfig, comparison_result: Dict[str, List[str]],
        ...
        if not source_data.empty:
            for _, row in source_data.iterrows():
                ...
                source_lookup[pk_key] = row.drop('rn', errors='ignore')
                
        if not target_data.empty:
            for _, row in target_data.iterrows():
                pk_key = "|".join(str(row[pk]) for pk in table_config.primary_key)
                target_lookup[pk_key] = row.drop('rn', errors='ignore')
